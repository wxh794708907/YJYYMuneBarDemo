...
//    [mainButton setBackgroundColor:[UIColor redColor]];
    [mainButton setTitle:@"菜单" forState:UIControlStateNormal];
    [mainButton setImage:[UIImage imageNamed:@"gallery"] forState:UIControlStateNormal];
...
-(void)showItems{
    if (!self.isShow) {
        ...
        [self.mainButton.layer addAnimation:scaleAnimation forKey:nil];
        CGFloat count = self.items.count;
        self.isShow = YES;
        ...
        switch (self.type) {
            ...
            case kMuneBarTypeLineLeft:{
                [self itemShowLineWithOffsetPoint:CGPointMake(self.frame.size.width / 2.0, self.frame.size.height / 2.0) incremenr:CGSizeMake(- self.frame.size.height - 30, 0)];
            }
                break;
...
-(void)tapItem:(CMuneItem *)item{
    ...
    if ([self.firstDelegate respondsToSelector:@selector(firstMuneBarselected:)]) {
        [self.firstDelegate firstMuneBarselected:item.tag - 100];
    }
